...
def main():
    ...
    # Show asset universe
    print(f"Asset Universe: {len(engine.asset_universe)} assets")
    ...
    for cluster, config in engine.asset_clusters.items():
        ...
        print(f"    Risk tolerance: {config['risk_tolerance']*100}") 
    ...
    if opportunities:
        ...
        for cluster, opps in cluster_opps.items():
            ...
            print(f"Cluster: {len(opps)} opportunities (avg conviction: {avg_conviction:.3})  # Test portfolio cycle
    print("\nPortfolio Cycle:")
    ...
    print("SUCCESS: Gemini's clustered strategy is working!")
    return True
