#!/usr/bin/env python3
"""
Test script for Optuna pause/resume functionality
"""
...
def test_optuna_pause_resume():
    """Test the pause/resume functionality with Optuna optimization"""
    
    # Setup pause/resume controller
    controller = setup_pause_resume()
    controller.set_callbacks(
        on_pause=lambda: logger.info("Training paused callback"),
        on_resume=lambda: logger.info("Training resumed callback"),
    ...
    logger.info("Press Ctrl+P to pause, Ctrl+R to resume")
    
    # Use the new optimize_with_pause_support function
    start_time = time.time()
    study = optimize_with_pause_support(study, objective, n_trials=20, timeout=300)
    end_time = time.time()
