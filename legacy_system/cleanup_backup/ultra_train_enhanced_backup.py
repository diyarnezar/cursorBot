...
class UltraEnhancedTrainer:
    ...
    def collect_enhanced_training_data(self, days: float = 0.083, minutes: int = None) -> pd.DataFrame:
        """Collect enhanced training data with 10X intelligence features, including whale features"""
        try:
            if minutes is not None:
                logger.info(f"📊 Collecting enhanced training data for {minutes} minutes...")
                # Calculate days needed for the minutes
                collection_days = max(1, int(minutes / 1440) + 1)  # 1440 minutes = 1 day
            else:
                logger.info(f"📊 Collecting enhanced training data for {days} days...")
            ...
            # Try to collect more data for better model training
            try:
                ...
                logger.warning(f"Comprehensive data collection failed: {e}, trying basic collection")
